...
describe('Quick Helpers', () => {
  it('Any raw Unitless Number assumes to be px ', () => {
    const { pxToEm,pxToRem, normalize,normalizeToEm,normalizeToRem ,pxToPct,pxToRelative} = styler
    expect(pxToEm(16)).toEqual('1em')
    expect(pxToRem(16)).toEqual('1rem')
    expect(pxToPct(16)).toEqual('100%')
    expect(pxToRelative(16)).toEqual(1)

    expect(normalize(16, 16,'em')).toEqual('1em')
  ...
  it('GetTheme', () => {
    ...
    expect(
      getTheme('customColors.black')).toEqual(undefined)
  })
